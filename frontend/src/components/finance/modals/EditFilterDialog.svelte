<script lang="ts">
  ...
  import type { BudgetCategory, BankTransactionFilter } from '@/lib/types/finance';

  export let filter: BankTransactionFilter | null = null;
  export let categories: BudgetCategory[] = [];

  let matches: string[] = filter?.matches || [];
...
<Dialog.Root>
  ...
  <Dialog.Content class="sm:max-w-[425px]">
    <Dialog.Header>
      ...
      <Dialog.Description>Update the filter's details and click "Save Changes."</Dialog.Description>
    </Dialog.Header>
        ...
        <select name="subcategoryId" class="mt-1 w-full rounded-lg border border-gray-300 p-2">
          ...
          {#each categories as category}
            {#each category.subcategories as subcategory}
              <option value={subcategory.id} selected={subcategory.id === filter?.subcategory_id}>
                {subcategory.name}
              </option>
            {/each}
          {/each}
