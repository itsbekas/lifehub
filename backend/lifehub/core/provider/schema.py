...
class OAuthProviderConfig(ProviderConfig):
    ...
    def build_auth_url(self) -> str:
        return f"{self.auth_url}?client_id={self.client_id}&redirect_uri={cfg.OAUTH_REDIRECT_URI}&scope={self.scope}&access_type=offline&response_type=code&state={self.provider_id}"

    def build_token_url(self, auth_code: str) -> str:
        return f"{self.token_url}?client_id={self.client_id}&redirect_uri={cfg.OAUTH_REDIRECT_URI}&scope={self.scope}&grant_type=authorization_code&client_secret={self.client_secret}&code={auth_code}"

    def build_refresh_token_url(self, refresh_token: str) -> str:
        return f"{self.token_url}?client_id={self.client_id}&redirect_uri={cfg.OAUTH_REDIRECT_URI}&scope={self.scope}&grant_type=refresh_token&client_secret={self.client_secret}&refresh_token={refresh_token}"
